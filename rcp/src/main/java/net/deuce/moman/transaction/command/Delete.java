...
import net.deuce.moman.transaction.ui.TransactionComposite;
...
public class Delete extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		ISelection selection = viewerRegistry.getViewer(
				TransactionComposite.TRANSACTION_VIEWER_NAME).getSelection();
		...
		if (MessageDialog.openQuestion(window.getShell(),
				"Delete Transaction?", "Are you sure you want to delete the "
						+ msg)) {
				...
				while (itr.hasNext()) {
					...
					if (!transaction.isInitialBalance()) {
						...
						if (transaction.isEnvelopeTransfer()) {
							...
							for (InternalTransaction it : importService
									.getEntities()) {
								if (it == transaction
										|| it.getMatchedTransaction() == transaction) {

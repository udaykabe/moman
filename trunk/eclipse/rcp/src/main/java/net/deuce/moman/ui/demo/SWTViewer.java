...
public class SWTViewer extends Canvas implements ICallBackNotifier,
		PaintListener {
	...
	public SWTViewer(Composite parent, int style) {
		...
		try {
            ...
            Envelope unassigned = envelopeService.getUnassignedEnvelope();
	...
	public Chart createChart() {
		...
		ChartWithoutAxesImpl cwaBar = (ChartWithoutAxesImpl) ChartWithoutAxesImpl
				.create();
	 ...
	 * 
	 * @see
	 * org.eclipse.birt.chart.device.IUpdateNotifier#removeContext(java.lang
	 * .Object)
	 */
	...
	public void callback(Object event, Object source, CallBackValue value) {
		if (source instanceof WrappedStructureSource) {
			...
			DataPointHints hint = (DataPointHints) ((WrappedStructureSource) source)
					.getSource();
			...
			for (Envelope env : envelope.getChildren()) {
				if (hint.getBaseDisplayValue()
						.equals(env.getChartLegendLabel())) {
			...
			if (refresh) {
				// Change title
				cm.getTitle().getLabel().getCaption().setValue(
						envelope.getChartLegendLabel());
	...
	public void addInteractivity(Chart cm, Class clazz) {
		...
		ActionValue actionValue = CallBackValueImpl.create(clazz
				.getCanonicalName());
		Action action = ActionImpl.create(ActionType.CALL_BACK_LITERAL,
				actionValue);
		Trigger onclick = TriggerImpl.create(TriggerCondition.ONCLICK_LITERAL,
				action);

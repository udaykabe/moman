...
public class DateRangeCombo extends Combo {
	...
	public DateRangeCombo(Composite parent, DateRange dateRange) {
		...
		addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				DateRangeCombo.this.dateRange = DateRange.values()[DateRangeCombo.this
						.getSelectionIndex()];
			}
	...
	/*
	 * 
	 * public Point computeSize(int wHint, int hHint, boolean changed) { Point p
	 * = super.computeSize(wHint, hHint, changed); int minHeight = 20*2 +
	 * getFont().getFontData()[0].getHeight(); System.out.println("ZZZ p.y: " +
	 * p.y); p.y = Math.max(minHeight, p.y); System.out.println("ZZZ p.y2: " +
	 * p.y); return p; }
	 */

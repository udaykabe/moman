...
public class RandomAccessLinkedListOFF<E extends AbstractEntity> extends
		AbstractSequentialList<E> implements List<E>, Deque<E>, Cloneable,
		java.io.Serializable {
	private transient Entry<E> header = new Entry<E>(null, null, null);
	private transient int size = 0;
	private transient Map<E, Entry<E>> entityMap = new HashMap<E, Entry<E>>();

	/**
	 * Constructs an empty list.
	 */
	public RandomAccessLinkedListOFF() {
	 ...
	 */
	public RandomAccessLinkedListOFF(Collection<? extends E> c) {
		this();
		addAll(c);
	}
	...
	private class ListItr implements ListIterator<E> {
		...
		public boolean hasNext() {
			Entry<E> nextEntry = nextEntry(next);
			return nextEntry != null && nextEntry.element != null;
		}
		...
		public boolean hasPrevious() {
			Entry<E> prevEntry = previousEntry(next);
			return prevEntry != header;
		}

		public E previous() {
			lastReturned = next = previousEntry(next);
			checkForComodification();
			return lastReturned.element;
		}
		...
		public void remove() {
			...
			/*
			 * checkForComodification(); Entry<E> lastNext = lastReturned.next;
			 * try { RandomAccessLinkedList.this.remove(lastReturned); } catch
			 * (NoSuchElementException e) { throw new IllegalStateException(); }
			 * if (next==lastReturned) next = lastNext; else nextIndex--;
			 * lastReturned = header; expectedModCount++;
			 */
		}
	...
	private E remove(Entry<E> e) {
		...
		size--;
		modCount++;
		entityMap.remove(result);
		return result;
	}

	protected boolean filter(E entity) {
		return false;
	}

	public E next(E entity) {
		Entry<E> entry = entityMap.get(entity);
		Entry<E> nextEntry = nextEntry(entry);
		return nextEntry != null ? nextEntry.element : null;
	}

	protected Entry<E> nextEntry(Entry<E> entry) {
		Entry<E> nextEntry = null;
		if (entry != null) {
			nextEntry = entry.next;
			while (nextEntry != null && nextEntry.element != null
					&& filter(nextEntry.element)) {
	...
	public E previous(E entity) {
		Entry<E> entry = entityMap.get(entity);
		Entry<E> previousEntry = previousEntry(entry);
		return previousEntry != null ? previousEntry.element : null;
	}

	protected Entry<E> previousEntry(Entry<E> entry) {
		Entry<E> previousEntry = null;
		if (entry != null) {
			previousEntry = entry.previous;
			while (previousEntry != null && previousEntry.element != null
					&& filter(previousEntry.element)) {
				previousEntry = previousEntry.previous;
			}
		}
	...
	/**
	 * Returns a shallow copy of this <tt>RandomAccessLinkedList</tt>. (The
	 * elements themselves are not cloned.)
	 * 
	 * @return a shallow copy of this <tt>RandomAccessLinkedList</tt> instance
	 */
	public Object clone() {
		RandomAccessLinkedListOFF<E> clone = null;
		try {
			clone = (RandomAccessLinkedListOFF<E>) super.clone();
	...
	/**
	 * Save the state of this <tt>RandomAccessLinkedList</tt> instance to a
	...
	/**
	 * Reconstitute this <tt>RandomAccessLinkedList</tt> instance from a stream

...
public class AvailableAccountsPage extends WizardPage implements Runnable {
				...
				.addPageChangedListener(new IPageChangedListener() {
					...
					public void pageChanged(PageChangedEvent event) {
						if (event.getSelectedPage() == AvailableAccountsPage.this) {
							...
							final FinancialInstitution fi = ((NewAccountWizard) getWizard())
								...
								getContainer().run(false, false,
										new IRunnableWithProgress() {
											...
											public void run(
												...
												monitor.beginTask(
														"Fetching Available accounts from "
																+ fi.getName(),
														100);
												monitor.worked(5);
												running = true;
							...
							if (exception != null) {
								...
								((NewAccountWizard) getWizard()).getContainer()
	...
	private void fetchAvailableAccounts() {
		...
		running = true;
		try {
			for (Account a : financialInstitutionService.getAvailableAccounts(

...
public abstract class AllocationDropAdapter extends DropTargetAdapter {
	...
	/**
	 * The last valid operation. We need to remember the last good operation in
	 * the case where the current operation temporarily is not valid (drag over
	 * someplace you can't drop).
	 */
	private int lastValidOperation;

	/**
	 * This is used because we allow the operation to be temporarily overridden
	 * (for example a move to a copy) for a drop that happens immediately after
	 * the operation is overridden.
	 */
	 ...
	 *            the viewer
	 */
	protected AllocationDropAdapter(Viewer viewer) {
		this.viewer = viewer;
	}
	...
	private void doDropValidation(DropTargetEvent event) {
		...
		currentEvent = event;
		overrideOperation = -1;
		if (!validateDrop(currentTarget, currentOperation,
	 ...
	 *             removed from DropTargetAdapter methods overridden by this
	 *             class. Handles any exception that occurs during callback,
	 *             including rethrowing behavior.
	 *             <p>
	 *             [Issue: Implementation prints stack trace and eats exception
	 *             to avoid crashing VA/J. Consider conditionalizing the
	 *             implementation to do one thing in VAJ and something more
	 *             reasonable in other operating environments. ]
	 *             </p>
	 * 
	 * @param exception
	 *            the exception
	...
	/**
	 * Overrides the current operation for a drop that happens immediately after
	 * the current validateDrop.
	 * 
